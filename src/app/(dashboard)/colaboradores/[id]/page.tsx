  ...
  return (
    <div>
      ...
      <div className="flex gap-1 mb-6 bg-gray-900 border border-gray-800 rounded-lg p-1 overflow-x-auto w-full md:w-fit">
        {(["dados", "plantoes", "feedbacks", "ocorrencias", "atestados"] as const).map(t => (
          <button key={t} onClick={() => setTab(t)}
            ...
            {t === "dados" ? "Dados pessoais" : t === "plantoes" ? `Plantões (${colab.plantoes.length})` : t === "feedbacks" ? `Feedbacks (${colab.feedbacks.length})` : t === "ocorrencias" ? `Ocorrências (${colab.ocorrencias.length})` : `Atestados (${colab.atestados.length})`}
          </button>
        ))}
      </div>
