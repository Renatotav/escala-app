...
export default function DnpjeMigracaoPage() {
  ...
  function load(pg = 1, buscaQ = "", filtroQ: string | null = null) {
    ...
    fetch(`/api/dnpje-migracao?${params}`)
      .then(r => r.json())
  ...
  async function handleImport() {
    ...
    for (let i = 0; i < selectedFiles.length; i++) {
      ...
      const res = await fetch("/api/dnpje-migracao/import", { method: "POST", body: formData });
  ...
  async function handleLimpar() {
    if (!confirm("Limpar todos os chamados DNPJE MIGRAÇÃO importados?")) return;
    await fetch("/api/dnpje-migracao", { method: "DELETE" });
    load();
  }
  ...
  async function exportXLS() {
    ...
    try {
      ...
      const res = await fetch(`/api/dnpje-migracao?${params}`);
      const data = await res.json();
      const todos: ChamadoDnpje[] = data.chamados ?? [];
      ...
      a.href = url; a.download = "dnpje_migracao.xls"; a.click();
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl w-full max-w-md p-6">
            ...
            <div className="mt-3 flex items-center gap-4">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="radio" name="modo-dnpje" checked={substituir} onChange={() => setSubstituir(true)} className="accent-blue-500" />
                <span className="text-sm text-gray-300">Substituir todos os dados</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="radio" name="modo-dnpje" checked={!substituir} onChange={() => setSubstituir(false)} className="accent-blue-500" />
