    ...
    <div>
      ...
      <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-x-auto">
        {loading ? (
          ...
          <div>
            <div className="flex items-center justify-between px-4 py-2 border-b border-gray-800 text-gray-400 text-xs uppercase tracking-wide">
              <span>Nº Chamado (Assyst)</span>
              <span>Status</span>
            </div>
            {semResolvido.length === 0 ? (
              <p className="px-4 py-8 text-center text-green-400 text-sm">Todos os chamados foram resolvidos!</p>
            ) : semResolvidoPag.map(num => (
              <div key={num} className="flex items-center justify-between px-4 py-2.5 border-b border-gray-800 last:border-0 bg-red-950/20 border-l-2 border-l-red-600 hover:bg-red-950/30 transition">
                ...
                <span className="inline-flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded-full bg-red-600 text-white animate-pulse whitespace-nowrap">
                  ⚠ Ainda não resolvido
                </span>
              </div>
            ))}
