...
export async function GET(request: NextRequest) {
  ...
  const [totalValidos, totalAtraso, totalAtencao, range] = await Promise.all([
    prisma.dnpjeMigracao.count({ where: VALIDO }),
    prisma.dnpjeMigracao.count({ where: { ...VALIDO, dataAbertura: { lt: d50 } } }),
    prisma.dnpjeMigracao.count({ where: { ...VALIDO, dataAbertura: { gte: d50, lt: d30 } } }),
    prisma.dnpjeMigracao.aggregate({ where: VALIDO, _min: { dataAbertura: true }, _max: { dataAbertura: true } }),
  ]);
  ...
  if (exportAll) {
    const chamados = await prisma.dnpjeMigracao.findMany({ where, orderBy: { dataAbertura: "asc" } });
    return NextResponse.json({ chamados, ...stats });
  }

  const [total, chamados] = await Promise.all([
    prisma.dnpjeMigracao.count({ where }),
    prisma.dnpjeMigracao.findMany({
      where,
...
export async function POST(request: NextRequest) {
  ...
  await prisma.dnpjeMigracao.createMany({
    data: rows.map(r => ({
      numero: r.numero,
...
export async function DELETE() {
  await prisma.dnpjeMigracao.deleteMany({});
  return NextResponse.json({ ok: true });
}
